...
Same as in previous method the conditioning information $m$ is acquired by sampling pixels. However, in this method, the conditioning information $m$ is sampled from the input image $x$ by sampling a variable amount of pixels from the input image $x$. The amount of pixels to be sampled from the input image will be a variable number of pixels.

The conditioning information $m$ has an added mask, which is used to tell the decoder which pixels are sampled from the input image $x$. The mask is a binary matrix of the same size as the input image $x$, where the value of 1 represents the sampled pixels and the value of 0 represents the pixels that are not sampled. When there is no conditioning information the mask is filled with zeros. The count sampling distribution is chosen to be a power law distribution with a high scaling factor so that the decoder can learn to handle also the cases where the conditioning information is not available. The scaling factor will be a hyperparameter of the model.
