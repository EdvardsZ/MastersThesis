...
In this section, I will present the results of \methodOne{1} on both Gaussian VAEs and VQ-VAEs. However, the results presented will be of the experiments conducted only on the CelebA dataset and Config. 2 for Gaussian VAEs and Config. 3 for VQ-VAEs. The experiments were conducted with both Exact Sampling and Uniform Sampling and tested with the SoftAdapt loss balancing technique and without it. The results on other configurations showed similar results with minor differences and can be found in the last section of this chapter~\ref{sec:cross_val_results}.
